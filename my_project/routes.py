...
def browse_page():
    ...
    if posts != []:             #if empty it should return the blank page with the heading + flash message
        return render_template("browse.html", page_tittle="Browse", posts=posts, users=users)   #render the template with posts= all post's and users= all users
    else:
...
def create_page():
    ...
    if form.validate_on_submit():   #chech validators
        ...
        db.session.commit()
        flash(f"Listening/Post created succesfully!", category="success")   #succes message and redirect to landing_page.html
        return redirect(url_for("landing_page"))
    if form.errors != {}:       #if validators fail => error message and returned to create.html
...
def mypost_page():
    posts = Post.query.filter_by(owner=current_user.id).all()   #get the post where owner ID is equal to current user ID
    if posts != []:
...
def register_page():
    ...
    if form.validate_on_submit():   #validators
        ...
        db.session.add(user_to_create)  #add and commit user_to_create to the Database, User
        db.session.commit()
...
def profile_page():
    ...
    image_file = url_for("static", filename="profile_pics/" + current_user.image_file)  #get the current user image file
# update the DB, User with the data from the ProfileForm
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            return render_template("profile.html", form=form)   #return the profile page with updated infos
        if (
            form.errors != {}
...
# TODO getting started / read more page
